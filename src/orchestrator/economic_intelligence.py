# -*- coding: utf-8 -*-
"""
Project Arkhē - Economic Intelligence Pipeline
진짜 경제적 지능: 정보 이론 + 승격 정책 + 실측 비용
"""
...
class InformationTheory:
    ...
    @staticmethod
    def js_divergence(samples1: List[str], samples2: List[str]) -> float:
        """Jensen-Shannon Divergence 계산 (numpy/scipy 선택적 사용)"""
        ...
        # 폴백: 간단한 대칭 KL divergence 근사
        divergence = 0.0
        for word in all_words:
            p = dist1.get(word, 0) + 1e-10  # smoothing
            q = dist2.get(word, 0) + 1e-10
            divergence += abs(p - q)
        
        return min(divergence, 1.0)  # [0, 1] 범위로 클램핑
...
class PromotionPolicy:
    ...
    def should_promote(self, samples: List[SampleResult], 
        ...
        # numpy 사용 가능하면 np.mean, 아니면 수동 계산
        if NUMPY_AVAILABLE:
...
class EconomicIntelligencePipeline:
    """진짜 경제적 지능 파이프라인"""
    ...
    def execute(self, query: str, llm_factory=None) -> EconomicIntelligenceResult:
        """경제적 지능 파이프라인 실행"""
        ...
        print(f"\n=== Economic Intelligence Pipeline: {query[:50]}... ===")
        ...
        if SCIPY_AVAILABLE:
            deps_status.append("SciPy+")
        if not deps_status:
            deps_status.append("BasicMode")
